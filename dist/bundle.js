...
/*!****************************!*\
  !*** ./src/Panelcanvas.ts ***!
  \****************************/
/*! exports provided: Panelcanvas */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "Panelcanvas", function() { return Panelcanvas; });
var Panelcanvas = /** @class */ (function () {
    /**
     * @param img nullであればepmty
     */
    function Panelcanvas(panelcount, index, w, h) {
...
/* harmony import */ var _Panelcanvas__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./Panelcanvas */ "./src/Panelcanvas.ts");

var Playarea = /** @class */ (function () {
    function Playarea(cssid_img, cssid_wrappanels, goalhandler) {
        var _this = this;
        /**
         * panelcanvasに設定するクリックハンドラ。移動。
         */
        this.evclickPanelcanvas = function (ev) {
            var index = parseInt(ev.target.dataset.index);
            _this.move(index);
            _this.drawCanvases();
            // ゴール判定
            if (_this.isGoal()) {
                document.querySelector(_this.cssid_wrappanels).style.display = "none";
                var img_1 = document.querySelector(_this.cssid_img);
                img_1.style.display = "inline";
                // アニメーション
                // アニメーション用のクラスを追加
                img_1.classList.add("beforemove");
                // 実行後、少ししてからアニメ発火
                setTimeout(function () {
                    img_1.classList.remove("beforemove");
                }, 100);
                _this.goalhandler();
            }
        };
     ...
     */
    Playarea.prototype.drawCanvases = function () {
        var canvases = document.querySelector(this.cssid_wrappanels);
        var panelcount = Math.sqrt(this.panelcanvases.length);
        // 前回のパネルは全て削除
        ...
        var nowidx = 0;
        // キャンバスを表示
        for (var r = 0; r < panelcount; r++) {
            var rowdiv = document.createElement("div");
            for (var c = 0; c < panelcount; c++) {
                for (var _i = 0, _a = this.panelcanvases; _i < _a.length; _i++) {
                    var cv = _a[_i];
    ...
    Playarea.prototype.start = function () {
        ...
        var canvases = document.querySelector(this.cssid_wrappanels);
        // パネルキャンバスを生成
        var pw = this.img.width / panelcount;
        var ph = this.img.height / panelcount;
        for (var i = 0; i < panelcount * panelcount; i++) {
            var cv = new _Panelcanvas__WEBPACK_IMPORTED_MODULE_0__["Panelcanvas"](panelcount, i, pw, ph);
        ...
        var nowemptyindex = (panelcount * panelcount) - 1;
        var premove = nowemptyindex; // 前回と同じパネルは動かさない。初回はありえないnowempty
        for (var i = 0; i < panelcount * panelcount * panelcount; i++) {
            // 全てのパネルから移動可能なパネルを抽出
            var movable = [];
            for (var _i = 0, _a = this.panelcanvases; _i < _a.length; _i++) {
                var pc = _a[_i];
                var nowcav = this.panelcanvases[nowemptyindex];
                // 前回のパネル以外かつ移動可能なもの
            ...
            // 今回移動するパネルを決定
            var moveindex = Math.floor(Math.random() * (movable.length));
     ...
     */
    Playarea.prototype.findEmptyPanelcanvasArri = function () {
        for (var i = 0; i < this.panelcanvases.length; i++) {
    ...
    Playarea.prototype.move = function (index) {
        // 現在のemptyパネルを取得
        var emptyi = this.findEmptyPanelcanvasArri();
        if (!this.panelcanvases[emptyi].isAdjoin(this.panelcanvases[index])) {
            // 接していないので移動しない
            return;
        }
        // 移動させるパネルとのindexを差し替え
        var swap = this.panelcanvases[index].position;
        ...
        // 実行後、少ししてからアニメ発火
        setTimeout(function () {
            var cv = document.querySelector(".panelcanvas.beforemove");
            cv.classList.remove("beforemove");
        }, 100);
    };
    Playarea.prototype.isGoal = function () {
        for (var _i = 0, _a = this.panelcanvases; _i < _a.length; _i++) {
            var cv = _a[_i];
...
/* harmony import */ var _Playarea__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./Playarea */ "./src/Playarea.ts");


var toastr = new toastr2__WEBPACK_IMPORTED_MODULE_0__["default"]();
var init = function (cssid_img, cssid_wrappanels) {
    var playarea = new _Playarea__WEBPACK_IMPORTED_MODULE_1__["Playarea"](cssid_img, cssid_wrappanels, function () {
